...
import { generateCaptcha, verifyCaptcha } from './server/captcha.ts';
import { generateToken, requireAuth, requireVdsOperator, hasVdsBindingPermission, checkServerPermission, AuthenticatedRequest } from './server/auth.ts';
import { MINECRAFT_LOADERS, ALL_MINECRAFT_VERSIONS, MinecraftLoader } from './server/minecraftLoaders.ts';
import { downloadAndDeployMinecraftServer, resolveOfficialJarUrl } from './server/minecraftDownloader.ts';
import {
  ...
  POPULAR_PLUGINS,
  POPULAR_MODS,
} from './server/minecraftService.ts';
import { testSmtpConnection, sendEmail } from './server/mailer.ts';
import { pterodactyl } from './server/pterodactylService.ts';
import {
  User,
...
async function startServer() {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    console.log(`[Flux Hosting] Minecraft Platform Server running on http://0.0.0.0:${PORT}`);
  });
}
